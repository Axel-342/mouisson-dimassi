...
let someCampaign = []
const removeCampaign = async (addHTML) => {
  await addHTML
  ...
  deleteButtons.forEach((deleteButton) => {
    deleteButton.addEventListener('click', () => {
      ...
      if (totalToRemove == 1) {
        ...
        someCampaign = fromJS.filter((campagne) => {
          if (
            closestData.dataset.id != campagne.selectedCampaign_id ||
        ...
        alert('Cette campagne a supprimé du panier')
        window.location.href = '../html/cart.html'
      }
    })
  })
  ...
  quantitys.forEach((quantity) => {
    quantity.addEventListener('change', () => {
      ...
      const foundSameOption = filtrage.find(
        (campagne) => campagne.option_campagne === closestData.dataset.color,
      )
