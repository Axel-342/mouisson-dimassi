  ...
  .then((res) => {
    ...
    btn_addToCart.addEventListener('click', (event) => {
      ...
      function addToArray() {
        ...
        window.alert("Le Campagne vient d'être ajouté à votre panier")
      }
      ...
      if (fromJSON) {
        ...
        for (i = 0; i < fromJS.length; i++){
          if (optionsCampagne.quantite === '0') {
            ...
            if (foundSameID.option_campagne === optionsCampagne.option_campagne) {// Si cet élément possède la même option produit
              let quantityToNumber = parseInt(optionsCampagne.quantite) // Nous convertissons la quantite du produit sélectionné en nombre
              ...
              foundSameID.option_campagne !== optionsCampagne.option_campagne
            ) {
              //Si le produit retourné par find possède bien le même ID, mais avec une option différente :
              const filtrage = fromJS.filter(
                (campagne) =>
                campagne.selectedCampaign_id ===
                  optionsCampagne.selectedCampaign_id,
              ...
              const filtrageFindOption = filtrage.find(
                (element) =>
                  element.option_campagne === optionsCampagne.option_campagne,
              ...
              if (filtrageFindOption === undefined) {
                ...
                const toNumber = parseInt(optionsCampagne.quantite)
                optionsCampagne.quantite = toNumber
                addToArray() // Alors nous ajoutons le produit au panier
              } else if (
                filtrageFindOption.option_campagne ===
                optionsCampagne.option_campagne
              ) {
                // Si find nous retourne un élément :
                let convertQuantity = parseInt(optionsCampagne.quantite) // Alors nous convertissons en number la quantite du produit séléctionné
